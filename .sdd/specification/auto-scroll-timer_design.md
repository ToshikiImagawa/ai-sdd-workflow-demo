...
| モジュール/機能                     | ステータス  | 備考                      |
|------------------------------|--------|-------------------------|
| useAutoSlideshow タイマーロジック追加  | 🔴 未実装 | 既存フックの拡張                |
| scrollSpeed 状態管理             | 🔴 未実装 |                         |
| SettingsWindow スクロールスピード設定UI | 🔴 未実装 | language-settings 実装に依存 |
| BroadcastChannel スクロールスピード同期 | 🔴 未実装 | 発表者ビューとの同期              |
...
| 設定UI     | MUI (TextField/Slider) | 既存プロジェクトがMUIを使用しており、一貫性を維持                                          |
...
type PresenterViewMessage =
  | { type: 'slideChanged'; index: number }
  | { type: 'controlStateChanged'; state: ControlState }
  | { type: 'navigate'; direction: 'prev' | 'next' }
  | { type: 'audioToggle' }
  | { type: 'autoPlayToggle' }
  | { type: 'autoSlideshowToggle' }
  | { type: 'scrollSpeedChange'; speed: number }  // 追加
```
