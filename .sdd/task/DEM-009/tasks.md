...
- [ ] `UseAudioPlayerReturn` インターフェースに `currentTime: number` と `duration: number` を追加
- [ ] `useAudioPlayer` フック内に `currentTime`、`duration` の state を追加
- [ ] `useEffect` 内で HTMLAudioElement の `timeupdate` イベントリスナーを登録し、`currentTime` と `duration` を更新
- [ ] `loadedmetadata` イベントで `duration` の初期値を設定
- [ ] クリーンアップ関数で `timeupdate`、`loadedmetadata` リスナーを解除
- [ ] `stop()` 呼び出し時に `currentTime` を 0、`duration` を 0 にリセット
...
- [ ] `ProgressBarProps` 型を定義（`progress: number`, `visible: boolean`）
- [ ] `ProgressBar` コンポーネントを実装
  - `visible` が `false` の場合は `null` を返す
  - `progress`（0.0〜1.0）を `width` のパーセンテージに変換
- [ ] `ProgressBar.module.css` を作成
...
- [ ] 型定義を作成: `ProgressSource`, `AudioProgress`, `TimerProgress`, `UseProgressBarOptions`, `UseProgressBarReturn`
- [ ] `useProgressBar` フックを実装
...
- [ ] `progress` を 0.0〜1.0 の範囲にクランプ
- [ ] `duration` や `total` が 0 の場合のゼロ除算を防止
...
- [ ] `App.tsx` で `useProgressBar` フックを呼び出す
...
- [ ] `SlideRenderer` の各スライド `<section>` 内に `ProgressBar` を配置
  - `SlideRenderer` に `progress`/`visible` props を追加するか、コンテキスト経由で渡す方式を検討
  - `position: absolute; bottom: 0` でスライド下部に固定
...
- [ ] `src/data/types.ts`: `PresenterViewMessage` に `progressChanged` メッセージ型を追加
  ``` typescript
...
- [ ] `src/App.tsx`: `useProgressBar` の戻り値が変化したら `sendProgressState` を呼び出す `useEffect` を追加
- [ ] `src/presenterViewEntry.tsx`（または発表者ビュー側のメッセージ受信部）:
  - `progressChanged` メッセージを受信して state に反映
- [ ] `src/components/PresenterViewWindow.tsx`:
...
- [ ] 実装ステータスを 🔴 未実装 → 🟢 実装済み に更新
- [ ] 各モジュールの実装進捗を更新
- [ ] 実装中に発生した設計判断があれば設計判断セクションに追記
